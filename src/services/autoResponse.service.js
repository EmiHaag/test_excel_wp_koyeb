        ...
        try {
            ...
            if (!rows || rows.length < 2) {
                ...
                if (keywordIdx === -1 || responseIdx === -1) {
                    ...
                    console.log(`📊 [AutoResponse] Índices de columnas: Keyword=${keywordIdx}, Response=${responseIdx}, Extra=${extraIdx}`);
                    this.responsesCache = rows.slice(1).map((row, i) => {
                        ...
                        const extraInfo = (extraIdx !== -1 && row[extraIdx]) ? String(row[extraIdx]).trim() : '';
